...
- **Sensitivity grid**: enterprise value across WACC × terminal growth, reported as premium/discount to the market EV.

## How to run

[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/pilatreanakin-code/Finance/blob/main/Reverse-DCF%20Monte%20Carlo%20Valuation%20%231/notebooks/demo.ipynb)
...
- `outputs/fair_value_distribution.png` — 20,000-draw fair-value histogram vs the market EV. On the synthetic demo the p5/p95 range is roughly 30,000–61,500 against a market EV of ~42,200 (P(undervalued) ≈ 49%): assumption uncertainty alone spans ~2× in value.
- `outputs/wacc_growth_sensitivity.png` — WACC × terminal-growth heatmap of premium/discount to market EV, making the point-estimate fragility explicit.
